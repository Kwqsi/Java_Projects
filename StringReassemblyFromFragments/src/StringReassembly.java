...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
         ...
         */
//make a substring of str2 using overlap for the parameter
        String substring = str2.substring(overlap);
        String combo = str1 + substring;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //looping through set
        while (temp.size() > 0) {
            String setString = temp.removeAny();
            //checks if the set contains Str and if it does not add the setString
            if (!setString.contains(str)) {
                strSet.add(setString);

            }
//            //if it does contain Str but is greater than Str
            else if (setString.length() > str.length()
                    && setString.contains(str)) {
                strSet.add(setString);
                check = false;
            }

        }
        //making a new temporary set to check if the setString is a subset of the Str
        Set<String> temp2 = strSet.newInstance();
        temp2.transferFrom(strSet);
        while (temp2.size() > 0) {
            String setString = temp2.removeAny();
            if (!str.contains(setString)) {
                strSet.add(setString);
            }
        }
        //if the boolean is true add the original string to the set
        if (check) {
            strSet.add(str);
        }
...
//while input isn't at the end
        while (!input.atEOS()) {
            //create  string for the next line and call addTo set to check prev. lines
            String current = input.nextLine();
            addToSetAvoidingSubstrings(set, current);
...
//for loop ending at the end of the text
        for (int i = 0; i < text.length(); i++) {
//if the char at i is ~ make a new line
            if (text.charAt(i) == '~') {
